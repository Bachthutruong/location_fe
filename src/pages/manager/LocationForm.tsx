...
const ManagerLocationForm = () => {
    ...
    const load = async () => {
      try {
        const [catRes, locRes] = await Promise.all([
          api.get('/categories'),
          isEdit ? api.get(`/locations/manager/${id}`) : Promise.resolve({ data: null })
        ])
        ...
        if (isEdit && locRes.data) {
          ...
          // store existing images separately for deletion control
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // keepImages for backend partial update
      if (isEdit) {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => navigate('/manager/locations')}>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  {isEdit ? '選擇新圖片以取代（不更動請留空）' : '請上傳至少一張圖片'}
                </p>
...
export default ManagerLocationForm
