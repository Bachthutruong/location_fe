...
const AdminNewsCategories = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingCategory) {
        await api.put(`/news-categories/${editingCategory._id}`, formData)
        toast.success('更新新聞分類成功！')
      } else {
        await api.post('/news-categories', formData)
        toast.success('新增新聞分類成功！')
      }
      handleCloseDialog()
      fetchCategories()
    } catch (error: any) {
      toast.error(error.response?.data?.message || '儲存新聞分類時發生錯誤')
    }
  }

  const handleOpenDeleteDialog = (category: NewsCategory) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await api.delete(`/news-categories/${categoryToDelete._id}`)
      toast.success('刪除新聞分類成功！')
      handleCloseDeleteDialog()
      fetchCategories()
    } catch (error: any) {
      toast.error(error.response?.data?.message || '刪除新聞分類時發生錯誤')
    }
  }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>可用的新聞分類</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingCategory ? '更新新聞分類資訊' : '新增新聞分類'}
            </DialogDescription>
          </DialogHeader>
